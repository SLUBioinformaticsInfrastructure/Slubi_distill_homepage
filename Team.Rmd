---
title: "Our team"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, out.width= "65%", out.extra='style="float:right; padding:10px"')
```


::: {.floatting}
```{r out.width='30%', out.extra='style="float:left; padding:20px"'}
knitr::include_graphics("images/Amrei_Binzer-Panchal.jpg")
```
<br><br><br>
[Amrei Binzer-Panchal, bioinformatician and coordinator at SLUBI (VH, Ultuna)](Amrei.html)
:::


::: {.floatting}
```{r out.width='30%', out.extra='style="float:right; padding:20px"'}
knitr::include_graphics("images/NicolasDelhomme_9704.jpg")
```
<br><br><br>
[Nicolas Delhomme, expert bioinformatician (S, Umeå)](Nicolas.html)
:::

::: {.floatting}
```{r out.width='30%', out.extra='style="float:left; padding:20px"'}
knitr::include_graphics("images/Varma.jpg")
```
<br><br><br>
[Ganapathi Varma Saripella (a.k.a. Varma), expert bioinformatician (LTV, Alnarp)](Varma.html)
:::


::: {.floatting}
```{r out.width='30%', out.extra='style="float:right; padding:20px"'}
knitr::include_graphics("images/AdnanNiazi.jpg")
```
<br><br><br>
[Adnan Niazi, expert bioinformatician (VH, Ultuna)](Adnan.html)
:::


::: {.floatting}
```{r out.width='30%', out.extra='style="float:left; padding:20px"'}
knitr::include_graphics("images/Abu.jpg")
```
<br><br><br>
[Abu Bakar Siddique, expert bioinformatician (NJ, Alnarp)](Abu_Bakar.html)
:::
